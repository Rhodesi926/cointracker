...
"""
    ...
    def aggregate_trades(self, trades: List[Dict]) -> pd.DataFrame:
        ...
        for trade in trades:
            ...
            token_stats[token_mint]['unique_kols'].add(trade['kol_name'])  # FIXED: was trade['wallet']
        ...
        for token_mint, stats in token_stats.items():
            results.append({
                ...
                'kol_names': ', '.join(sorted(stats['kol_names'])),  # ADDED: sorted for consistency
            })
...
def main():
    try:
        ...
        results = tracker.run_analysis(hours_back=72)
        
        if not results.empty:
            print("\nTOP KOL PICKS (Last 48 hours):")
