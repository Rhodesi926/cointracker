...
class PatternCoordinationAnalyzer:
    ...
    def find_systematic_front_runners(self, gake_purchases: List[Dict], min_pattern_count: int = 2) -> Dict:
        ...
        for i, purchase in enumerate(gake_purchases):  # Analyze ALL GAKE purchases
            print(f"Scanning holders for token {i+1}/{len(gake_purchases)}: {purchase['token_symbol']}")
    ...
    def save_extended_results(self, systematic_patterns, timing_anomalies, network_effects, 
                            second_degree, liquidity_coordination, volume_patterns):
        """Save EXTENDED analysis results to multiple files"""
            ...
            primary_df = pd.DataFrame([
                {
                    ...
                    'coordination_count': data['coordination_count'],
                    'coordination_rate_percent': data['coordination_rate'],
                    'avg_lead_time_hours': data['avg_lead_time_hours'],
                    'total_volume': data['total_coordinated_volume'],
                    'is_liquidity_provider': wallet in liquidity_coordination,
                    'network_connections': len(network_effects.get(wallet, {}).get('shared_tokens_with_other_coordinated_wallets', []))
                }
                for wallet, data in systematic_patterns.items()
            ])
            primary_df.to_csv('primary_coordination.csv', index=False)
        ...
        if second_degree:
            secondary_df = pd.DataFrame([
                {
                    'wallet_address': wallet,
                    'coordination_matches': data['total_second_degree_matches'],
                    'primary_wallet_connections': len(data['coordinates_with'])
                }
                for wallet, data in second_degree.items()
            ])
            secondary_df.to_csv('secondary_coordination.csv', index=False)
        ...
        if network_effects:
            ...
            for wallet1, connections in network_effects.items():
                for connection in connections['shared_tokens_with_other_coordinated_wallets']:
                    network_rows.append({
                        'wallet_1': wallet1,
                        'wallet_2': connection['partner_wallet'],
                        'shared_tokens': connection['shared_token_count'],
                        'coordination_rate': connection['coordination_rate'] * 100
                    })
            
            if network_rows:
                network_df = pd.DataFrame(network_rows)
                network_df.to_csv('network_relationships.csv', index=False)
            ...
            patterns_df = pd.DataFrame([
                {
                    ...
                    'avg_lead_time_hours': data['avg_lead_time_hours'],
                    'total_volume': data['total_coordinated_volume'],
                    'size_similarity': volume_patterns.get(wallet, {}).get('size_similarity_to_gake', 0) * 100,
                    'consistent_sizing': volume_patterns.get(wallet, {}).get('consistent_sizing', False)
                }
                for wallet, data in systematic_patterns.items()
            ])
            patterns_df.to_csv('systematic_coordination.csv', index=False)
            ...
            anomalies_df = pd.DataFrame([
                {
                    ...
                    'wallets_6h_before': data['buys_6h_before'],
                    'wallets_24h_before': data['buys_24h_before'],
                    'total_wallets': data['total_wallets_in_window']
                }
                for token_mint, data in timing_anomalies.items()
            ])
            anomalies_df.to_csv('timing_anomalies.csv', index=False)
